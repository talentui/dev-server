    ...
    renderConfigList() {
        return this.props.data.map(item => {
            ...
            return <div key={id} className='config-item'>
                    ...
                    <input type='text' value={item.get('port')} onChange={this.handleChangePort(id)} placeholder='端口号' className='config-input-left' />-
                    <input type='text' value={item.get('referer')} onChange={this.handleChangeReferer(id)} placeholder='引用地址' className='config-input-left' />-
                    <input type='text' value={item.get('reg')} onChange={this.handleChangeReg(id)} placeholder='配置规则' className='config-input-right short' />
                    <button className='delete' onClick={this.handleDeleteConfig(id)} >删除</button>
            </div>
        })
    }

    render() {
        return <section className='special-config config-section'>
